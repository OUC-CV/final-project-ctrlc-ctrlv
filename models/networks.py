...
class CrossEntropyMaskedLoss(nn.Module):
    def __init__(self):
        super(CrossEntropyMaskedLoss, self).__init__()
        self.cross_entropy_loss = nn.CrossEntropyLoss(reduction='none')  # 不进行归约，以便之后应用掩码

    def forward(self, pred, label, mask=None):
        # pred: [batch_size, num_classes, height, width]
        # label: [batch_size, height, width]
        # mask: [batch_size, height, width] 或 None
        
        if mask is None:
            mask = torch.ones_like(label, dtype=torch.float32)  # 默认掩码全为1
...
class L2MuLoss(nn.Module):
    def __init__(self, mu=5000):
        super(L2MuLoss, self).__init__()
        self.mu = mu
        self.l2_loss = nn.MSELoss()  # 使用均方误差损失

    def forward(self, pred, label, mask=1.):
        mu_pred = range_compressor_cuda(pred, self.mu) * mask
        mu_label = range_compressor_cuda(label, self.mu) * mask
        return self.l2_loss(mu_pred, mu_label)
