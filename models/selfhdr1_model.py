...
#            self.optimizer_netSelfHDR = optim.SGD(self.netSelfHDR.parameters(),
#                                         lr=opt.lr,
...
#                                         weight_decay=opt.weight_decay)
            ...
            self.criterionL2 = N.init_net(N.L2MuLoss(), gpu_ids=opt.gpu_ids)
            self.criterionCrossEntropy = N.init_net(N.CrossEntropyMaskedLoss(), gpu_ids=opt.gpu_ids)
...
#        self.loss_StruExpan = self.criterionCrossEntropy(self.data_out, self.data_color_label, label_mask).mean()
#        self.loss_StruPrese = self.criterionCrossEntropy(self.data_out, self.data_input1[:, 0:3, ...], in_mask).mean() * 4
        self.loss_StruExpan = self.criterionL2(self.data_out, self.data_color_label, label_mask).mean()  # Structure-Expansion
        self.loss_StruPrese = self.criterionL2(self.data_out, self.data_input1[:, 0:3, ...], in_mask).mean() * 4  # Structure-Preserving
